...
double flow_distance(flow* F1, flow* F2, double WB, double WT, double WD, double WS) {
    ...
    return sqrt(B*B*WB + T*T*WT + D*D*WD + S*S*WS);
}

//find cluster distance (minimum distance of flows between clusters)
double cluster_distance(cluster* clusterA, cluster* clusterB, double WB, double WT, double WD, double WS) {
...
int process_clusters(cluster* clusters, int clusters_size);
...
int main(int argc, char* argv[]) { //syntax: ./flows SOUBOR [N WB WT WD WS]
    ...
    // Read args
    int DESIRED_CLUST_SIZE = 0;
    double WB = 1; double WT = 1; double WD = 1; double WS = 1;
    if (argc == 7) {
        DESIRED_CLUST_SIZE = atoi(argv[2]);
    ...
    //INPUT 1 - flow count
    int flow_count = 0;
    if (fscanf(DATA, "count=%i", &flow_count)){}
    if (flow_count <= 0) {
    ...
    //PROCESS CLUSTERS
    while (clusters_size > DESIRED_CLUST_SIZE && DESIRED_CLUST_SIZE > 0){
        ...
        realloc_temp = realloc(clusters, clusters_size * sizeof(cluster)); 
        if (realloc_temp == NULL) {
